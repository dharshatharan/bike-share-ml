...
dayofweek(start_time::date) as day_of_week,
        ...
        case when dayofweek(start_time::date) in (6, 7) then true else false end as is_weekend,
...
count(*) as total_trips__leaky,
        count(*) filter (where user_type = 'Annual Member') as annual_member_trips__leaky,
        count(*) filter (where user_type = 'Casual Member') as casual_member_trips__leaky,
        count(*) filter (where user_type = 'Annual Member')::double / nullif(count(*), 0)::double as annual_member_ratio__leaky,
...
station_rankings as (
    select
        ...
        end_station_id,
        count(*) as trip_count,
        row_number() over (partition by start_time::date order by count(*) desc) as start_station_rank,
        row_number() over (partition by start_time::date order by count(*) desc) as end_station_rank
    from {{ ref('int_combined_trip_data') }}
...
case
            ...
            then ((total_trips__leaky - lag(total_trips__leaky, 1) over (order by trip_date))::double 
                  / lag(total_trips__leaky, 1) over (order by trip_date)::double) * 100
            else null
        end as trips_change_1d_pct,
        
        case
            when lag(total_trips__leaky, 7) over (order by trip_date) > 0
            then ((total_trips__leaky - lag(total_trips__leaky, 7) over (order by trip_date))::double 
                  / lag(total_trips__leaky, 7) over (order by trip_date)::double) * 100
            else null
        end as trips_change_7d_pct,
        
        case
            when lag(total_trips__leaky, 30) over (order by trip_date) > 0
            then ((total_trips__leaky - lag(total_trips__leaky, 30) over (order by trip_date))::double 
                  / lag(total_trips__leaky, 30) over (order by trip_date)::double) * 100
            else null
...
select * from lagged_features order by trip_date